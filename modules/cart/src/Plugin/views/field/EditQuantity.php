...
class EditQuantity extends FieldPluginBase {
  ...
  public function viewsForm(array &$form, FormStateInterface $form_state) {
    return $this->viewsFormOverride($form, $form_state);
   ...
   * Overrides ::viewsForm().
   */
  public function viewsFormOverride(array &$form, FormStateInterface $form_state) {
    // Make sure we do not accidentally cache this form.
    $form['#cache']['max-age'] = 0;
    // The view is empty, abort.
    if (empty($this->view->result)) {
  ...
  public function viewsFormSubmit(array &$form, FormStateInterface $form_state) {
    return $this->viewsFormSubmitOverride($form, $form_state);
  ...
  /**
   * Overrides ::viewsFormSubmit().
   */
  public function viewsFormSubmitOverride(array &$form, FormStateInterface $form_state) {
    $quantities = $form_state->getValue($this->options['id'], []);
    foreach ($quantities as $row_index => $quantity) {
      /** @var \Drupal\commerce_order\Entity\OrderItemInterface $order_item */
      $order_item = $this->getEntity($this->view->result[$row_index]);
